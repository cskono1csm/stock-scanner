...
TOP_N = 30
OUTPUT_DIR = "output"
HISTORY_PATH = "docs/data/history.json"  # 추천종목 성과추적용 이력 파일(저장소에 커밋되어 누적됨)

# 성과추적 대상: 대시보드의 5개 TOP 리스트와 동일한 기준(섹션 라벨, 정렬 기준 컬럼, 개수, 최소 모델수)
RECOMMEND_SECTIONS = [
    ("종합", "종합점수", 20, 0),
    ("가치주", "가치점수", 15, 0),
    ("모멘텀", "모멘텀점수", 15, 0),
...
def _load_history(path=HISTORY_PATH) -> dict:
...
def _save_history(history: dict, path=HISTORY_PATH):
    os.makedirs(os.path.dirname(path) or ".", exist_ok=True)
    with open(path, "w", encoding="utf-8") as f:
        json.dump(history, f, ensure_ascii=False)


def _todays_top_picks(df: pd.DataFrame) -> dict:
    """오늘 각 TOP 리스트(종합/가치주/모멘텀/배당/적정주가)에 오른 종목을 모읍니다.
    반환값: {"KR_005930": "종합", ...} - 여러 섹션에 동시에 들 경우 먼저 매칭된 섹션 라벨만 기록합니다."""
    picks = {}
    for label, sort_col, n, min_models in RECOMMEND_SECTIONS:
...
def update_recommendation_history(df: pd.DataFrame, path=HISTORY_PATH) -> dict:
    """오늘 새로 TOP 리스트에 오른 종목을 추천 이력에 추가합니다(이미 기록된 종목은 최초 추천일/추천가를
    그대로 유지합니다). 이렇게 저장소에 누적되는 이력(docs/data/history.json)을 바탕으로 대시보드에서
    '추천일 대비 현재가' 성과를 보여줍니다. 이 파일이 생기기 전 과거 추천은 소급 기록할 수 없어,
    이 기능을 배포한 시점부터 새로 쌓이기 시작합니다."""
    history = _load_history(path)
    today = dt.date.today().isoformat()
    picks = _todays_top_picks(df)
    row_by_key = {_card_key(r): r for _, r in df.iterrows()} if not df.empty else {}
    added = 0
    for key, label in picks.items():
        ...
        added += 1
    if added:
        print(f"[이력] 신규 추천 종목 {added}개 기록됨 (누적 {len(history)}개)")
    _save_history(history, path)
    return history


def _fmt_ret_pct(v, digits=1) -> str:
    if v is None or (isinstance(v, float) and pd.isna(v)):
        return "-"
    return f"{v:+.{digits}f}%"


def _perf_row(entry: dict, current_price) -> str:
    market = entry.get("시장구분", "US")
    name = html.escape(str(entry.get("종목명", "")))
    ticker = html.escape(str(entry.get("티커", "")))
    label = html.escape(str(entry.get("추천구분", "")))
    rec_date = entry.get("추천일", "-")
    rec_price = entry.get("추천가")
    has_now = current_price is not None and not (isinstance(current_price, float) and pd.isna(current_price))
    ret = None
    if has_now and rec_price:
        ret = (current_price / rec_price - 1) * 100
    try:
        days = (dt.date.today() - dt.date.fromisoformat(rec_date)).days
    except Exception:
        days = None
    days_str = f"{days}일 경과" if days is not None else "-"
    ret_str = _fmt_ret_pct(ret)
    ret_cls = "pos" if (ret is not None and ret > 0) else ("neg" if (ret is not None and ret < 0) else "")
    now_str = _fmt_money(current_price, market, is_price=True) if has_now else "데이터없음"
    rec_str = _fmt_money(rec_price, market, is_price=True)
    return (
        f'<tr data-market="{market}"><td><span class="pname">{name}</span>'
        f'<span class="pticker">{ticker} · {label}</span></td>'
        f'<td>{html.escape(str(rec_date))}<span class="pticker">{days_str}</span></td>'
        f'<td>{rec_str}</td><td>{now_str}</td>'
        f'<td class="{ret_cls}">{ret_str}</td></tr>'
    )


def _build_performance_section(df: pd.DataFrame, history: dict) -> str:
    if not history:
        return ('<section id="performance"><h2>\U0001F4C8 추천종목 성과추적</h2>'
                '<p class="empty">아직 누적된 추천 이력이 없습니다. 다음 갱신부터 하나씩 쌓입니다.</p></section>')
    price_by_key = {}
    if not df.empty:
        for _, r in df.iterrows():
            price_by_key[_card_key(r)] = r.get("현재주가")

    def _days_elapsed(entry):
        try:
            return (dt.date.today() - dt.date.fromisoformat(entry.get("추천일", ""))).days
        except Exception:
            return -1

    groups = []
    for market, label in (("KR", "\U0001F1F0\U0001F1F7 한국"), ("US", "\U0001F1FA\U0001F1F8 미국")):
        entries = [(k, e) for k, e in history.items() if e.get("시장구분") == market]
        # 추천된 지 오래된(경과일이 긴) 순서 - 실적 검증 가치가 큰 종목을 먼저 보여줌
        entries.sort(key=lambda item: _days_elapsed(item[1]), reverse=True)
        if not entries:
            body_html = '<p class="empty">데이터 없음</p>'
        else:
            body_html = (
                '<div class="perf-scroll"><table class="perf-table"><thead><tr>'
                '<th>종목</th><th>추천일</th><th>추천가</th><th>현재가</th><th>수익률</th>'
                '</tr></thead><tbody>'
                + "".join(_perf_row(e, price_by_key.get(k)) for k, e in entries)
                + '</tbody></table></div>'
            )
        groups.append(
            f'<div class="market-group" data-market="{market}">'
            f'<h3 class="market-h">{label} ({len(entries)}개)</h3>{body_html}</div>'
        )
    return f'<section id="performance"><h2>\U0001F4C8 추천종목 성과추적</h2>{"".join(groups)}</section>'
...
def build_dashboard(df: pd.DataFrame, history: dict = None, out_path: str = "docs/index.html") -> str:
    ...
    perf = _build_performance_section(df, history or {})
...
<nav><a href="#top">종합</a><a href="#value">가치주</a><a href="#momentum">모멘텀</a><a href="#dividend">배당</a><a href="#fairvalue">적정주가</a><a href="#performance">성과추적</a></nav>
<div class="filterbar">
...
<main>{total}{value}{mom}{div_}{fair}{perf}</main>
...
"추천종목 성과추적"의 추천일은 해당 종목이 TOP 리스트에 처음 오른 날짜이며, 이 기능을 배포한
시점부터 새로 누적됩니다(과거 추천은 소급 기록되지 않습니다).<br><br>
...
def run(kr: bool = True, us: bool = True) -> str:
    ...
    history = update_recommendation_history(combined)

    xlsx_path = export(combined)
    build_dashboard(combined, history, out_path="docs/index.html")

...
def _fetch_naver_valuation(code: str) -> dict:
    ...
    if pd.notna(result["PER"]) or pd.notna(result["PBR"]):
        return result  # 모바일 API로 핵심 값(PER/PBR)을 확보했으면 데스크톱 폴백 불필요
...
def build_kr_universe():
    ...
    print(f"[KR] 시가총액 최종 필터 직전: {len(fundamentals)}개 종목 중 시가총액 확보 "
          f"{int(cap_valid.notna().sum())}개, {KR_MIN_MARKETCAP:,.0f}원 이상 "

...
# 2026.09.14: 시가총액 데이터 파이프라인(FinanceDataReader + 네이버 폴백)이 사실상 전멸해서
# (예: 2765개 중 유효값 0개) KR_MIN_MARKETCAP 필터를 적용하면 한국 종목이 통째로 0개가 되는
# 상황이 실제로 발생했습니다. 이럴 때는 필터를 아예 건너뛰고 "일단 몇 종목이라도 보여주는"
# 비상 우회 경로를 타는데, fetch_kr_momentum()이 종목별 순차 조회(+ time.sleep)라서 원래는
# 시가총액 필터로 2,700여 개를 수백 개까지 미리 추려낸 뒤에만 호출되는 것을 전제로 시간 예산이
# 짜여 있습니다. 필터를 완전히 건너뛰면 그 전제가 깨져 예전에 있었던 GitHub Actions 45분
# 타임아웃이 재발할 위험이 있으므로, 비상 경로에서도 최종 종목 수는 이 값으로 상한을 둡니다.
KR_EMERGENCY_MAX_UNIVERSE = 500
...
def _to_numeric_clean(series) -> pd.Series:
    """콤마·원화 기호·공백이 섞인 문자열(예: "1,234,567", "1,234,567원")이 껴 있어도 숫자로
    변환합니다. 2026.09.14: 한국 종목 시가총액이 FinanceDataReader에서 전종목 0/2765개로
    확보되는 장애가 실제로 있었는데, 라이브 접근이 안 돼 정확한 원인(KRX 소스 자체 포맷 변경
    가능성 등)을 확정하지 못했습니다 - 흔한 원인 중 하나인 "콤마가 섞인 문자열이라
    pd.to_numeric이 그대로 NaN 처리"하는 경우를 방어적으로 미리 막아둡니다. 이미 숫자 타입이면
    문자열 변환 없이 그대로 통과합니다(부작용 없음)."""
    if series is None:
...
def get_kr_ticker_universe() -> pd.DataFrame:
    ...
    for m in KR_MARKETS:
        ...
        if code_col is None or name_col is None:
            ...
            continue
        # 2026.09.14: 시가총액이 전 종목 0/NaN으로 나와 이후 필터에서 종목이 통째로 다 걸러지는
        # (심하면 build_kr_universe가 빈 DataFrame을 받는) 문제를 진단하기 위해, 어떤 경로로
        # 리스트를 확보했는지와 원본 시가총액 컬럼 값 예시를 always 로그로 남깁니다.
        print(f"[KR] {m}: cap_col={cap_col!r}, 원본 값 예시(최대 3개)="
              f"{listing[cap_col].head(3).tolist() if cap_col else '컬럼 없음(KIND 대체 경로 등)'}")
    ...
    if "시가총액_참고" in combined.columns:
        ...
        # 시가총액 필터 단계에서 걸러짐).
        cap_num = _to_numeric_clean(combined["시가총액_참고"])
        combined = combined[cap_num.isna() | (cap_num >= KR_MIN_MARKETCAP)]
        print(f"[KR] 종목 리스트 {before}개 -> 시가총액 사전 필터 후 {len(combined)}개 "
              f"(원본 시가총액 값 중 숫자로 인식된 것 {int(cap_num.notna().sum())}개/{before}개)")
    return combined.reset_index(drop=True)


# 2026.09.14 (3): 실제 GitHub Actions 로그(44분 52초에 "The operation was canceled." -
# screen.yml의 job timeout-minutes: 45에 걸려 워크플로 자체가 강제 취소된 것으로 확인)에서,
# 네이버 밸류에이션/사업개요/재무비율 3단계가 각각 최대 NAVER_FETCH_TIME_BUDGET_SEC(기존
# 900초=15분)까지 순차적으로 소진할 수 있어 세 단계만으로 최악의 경우 45분(3×15분)을 그대로
# 다 써버려, 그 뒤에 와야 할 모멘텀 수집·미국 처리·대시보드 생성이 시작도 못 해보고 워크플로
# 시간제한에 걸릴 위험이 실제로 있었습니다(이번 실행이 정확히 이 패턴 - 사업개요 단계에서
# "시간 예산 900초 초과로 46개 종목 건너뜀" 로그가 찍혔고, 그 다음 재무비율 단계 중간에
# 취소됨). 두 가지로 완화합니다: (1) 단계당 시간 예산을 480초(8분)로 줄여 3단계 합계 상한을
# 24분으로 낮추고, (2) 동시 요청 수를 10→20으로 늘려(순수 네트워크 대기 위주라 스레드를
# 늘리는 부담이 적음) 같은 예산 안에 더 많은 종목을 처리하게 합니다. 그래도 실제 네트워크
# 장애(호스트 자체가 느리거나 간헐적으로 응답 없음) 자체를 없애주지는 못하므로, 워크플로의
# timeout-minutes 자체도 45분보다 넉넉하게(예: 75분) 늘리는 것을 별도로 권장합니다(screen.yml
# 수정 필요 - main.py만으로는 고칠 수 없는 부분).
NAVER_MAX_WORKERS = 20
NAVER_FETCH_TIME_BUDGET_SEC = 480  # 8분 - 이 시간을 넘기면 남은 종목은 건너뛰고 계속 진행
...
_naver_debug_count = 0
_NAVER_DEBUG_LIMIT = 3  # 첫 N개 종목까지 상세 로그(우연/일시적 실패와 구조적 실패 구분용)

# 다른 Naver 계열 스크래핑 함수(get_sp500_tickers, fetch_macro_indicators)와 동일한 브라우저
# User-Agent. 2026.09.13 라이브 점검에서 삼성전자를 포함해 거의 모든 한국 종목의 PER/PBR/EPS/
# BPS/배당수익률이 이 함수에서 전부 비어 있는 것이 확인됐는데, 같은 시점에 WiseReport
# (navercomp.wisereport.co.kr, 사업개요 스크래핑)에서도 데이터가 안 잡히고 있어(서로 다른
# 도메인 두 곳이 동시에 실패) User-Agent가 없는 기본 requests 요청 자체가 (GitHub Actions
# 러너의 데이터센터 IP 대역에 대해) 광범위하게 차단됐을 가능성이 가장 유력한 것으로 판단해
# 추가합니다 - FRED 매크로 지표에서 이미 같은 유형의 문제를 실제로 확인한 바 있습니다.
_BROWSER_HEADERS = {
    "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 "
                  "(KHTML, like Gecko) Chrome/124.0.0.0 Safari/537.36",
}
...
_EMPTY_NAVER_VALUATION = {
    "PER": np.nan, "PBR": np.nan, "EPS": np.nan, "BPS": np.nan,
    "DIV": np.nan, "시가총액_naver": np.nan, "상장주식수_naver": np.nan,
}

# 2026.09.13 라이브 점검 + 2026.09.14 사용자 공유 로그로 확인된 사실: finance.naver.com/item/
# main.naver가 Next.js 기반 클라이언트 렌더링 페이지로 바뀌어(응답은 status=200·정상 길이로
# 오지만 <em id="_per"> 같은 값 태그 자체가 서버 HTML에 없고 _next/static 스크립트가 브라우저에서
# 그려줌) 기존 방식으로는 PER/PBR/EPS/BPS를 원천적으로 가져올 수 없게 됐습니다(User-Agent 문제가
# 아니라 사이트 구조 자체가 바뀐 것). 네이버가 자사 모바일 앱/페이지에는 여전히 서버가 값을 채워
# 내려주는 JSON API(m.stock.naver.com)를 쓰는 것으로 알려져 있어 이를 1순위로 시도합니다.
# **주의: 이 API 응답 스키마는 이 세션에서 라이브로 검증하지 못했습니다**(m.stock.naver.com도
# 프록시가 차단) - 흔히 쓰이는 필드명(코드값 "per"/"pbr"/"eps"/"bps")을 최선으로 추정해
# 파싱하고, 실패하면 기존 데스크톱 페이지 방식으로 폴백합니다. 스키마가 추정과 다르면 계속
# 실패할 수 있는데, 그 경우를 대비해 첫 3개 종목은 원본 JSON을 그대로 로그에 남겨(다음 실행
# 로그로) 정확한 필드명을 확인해 다시 고칠 수 있게 합니다.
def _parse_krw_compound(s: str):
    """"148조 6,794억"처럼 조/억 단위가 섞인 한국식 큰 숫자 표기를 원(won) 단위 숫자로
    변환합니다. 2026.09.14 실제 GitHub Actions 로그로 확인된 m.stock.naver.com 모바일 API의
    시가총액("시총") 필드가 정확히 이 형식으로 옵니다(예: "148조 6,794억" = 삼성전자 시가총액).
    "조"나 "억" 중 하나만 있어도(예: "6,794억") 처리하며, 둘 다 없으면 None을 반환해 호출부가
    "이 형식이 아니다"를 구분할 수 있게 합니다."""
    s = s.replace(",", "").strip()
    total = 0.0
    found = False
    ...
    if m_jo:
        try:
            total += float(m_jo.group(1)) * 1_000_000_000_000
            found = True
        except ValueError:
            pass
    ...
    if m_eok:
        try:
            total += float(m_eok.group(1)) * 100_000_000
            found = True
        except ValueError:
            pass
...
def _fetch_naver_valuation_mobile_api(code: str, should_debug: bool) -> dict:
    result = dict(_EMPTY_NAVER_VALUATION)
    try:
        resp = requests.get(
        ...
        resp.raise_for_status()
        data = resp.json()
        total_infos = data.get("totalInfos") or data.get("totalInfo") or []
        field_map = {}
        if isinstance(total_infos, list):
            for item in total_infos:
                if isinstance(item, dict) and item.get("code"):
                    field_map[str(item["code"]).lower()] = item.get("value")
        ...
        def _f(*keys):
            for k in keys:
                ...
                if v is None:
                    continue
                # 2026.09.14 실제 로그로 확인: 이 API의 값 문자열은 "8.31배"(PER/PBR)나
                # "22,292원"(EPS/BPS)처럼 콤마·퍼센트뿐 아니라 "배"/"원" 같은 한글 단위가 붙어
                # 오는데, 이전 코드는 콤마·퍼센트만 제거해 float() 변환이 항상 실패하고 있었습니다
                # (필드 자체는 정상적으로 찾았는데 숫자 변환에서만 조용히 실패 - 로그에서 실제
                # totalInfos에 'per'/'pbr'/'eps'/'bps' 키가 다 보이는데도 결과가 전부 NaN으로
                # 나온 것으로 확인된 버그). 알려진 단위 접미사를 모두 제거한 뒤 변환합니다.
                cleaned = (
                    str(v).replace(",", "").replace("%", "")
                    .replace("배", "").replace("원", "").replace("주", "").strip()
                )
                try:
        ...
        result["EPS"] = _f("eps")
        result["BPS"] = _f("bps")
        result["DIV"] = _f("dvr", "dividendyieldratio", "divrate")
        # 시가총액도 이 응답 어딘가에 있을 가능성이 높아 최선을 다해 추정 시도합니다. 2026.09.14
        # 실제 로그로 code="marketValue"(totalInfos 안, key="시총") 필드가 실제 존재함을
        # 확인했으나 값이 "148조 6,794억"처럼 조/억 복합 표기라 단순 float() 변환이 아예
        # 불가능했던 것이 이전 코드에서 시가총액_naver가 항상 NaN이었던 원인입니다. 우선
        # 조/억 복합 표기 파서로 시도하고, 실패하면(이미 순수 숫자로 오는 경우 대비) 기존
        # 방식(콤마만 제거 후 float)으로도 시도합니다.
        market_cap = None
        for k in ("marketvalue", "totalmarketvalue", "marcap", "aftermarketvalue"):
            v = field_map.get(k)
            if v is None:
                continue
            parsed = _parse_krw_compound(str(v))
            if parsed is not None:
                market_cap = parsed
                break
            try:
                # 조/억 복합 표기가 아니라 순수 숫자로 오는 경우(관측된 적은 없으나 대비):
                # 자릿수가 시가총액치고 비정상적으로 작으면(1e6 미만) "억원" 단위로 보고
                # 원 단위로 환산합니다(2026.09.14 이전 코드의 방어 로직을 그대로 유지).
                raw = float(str(v).replace(",", "").replace("원", ""))
                market_cap = raw * 100_000_000 if raw < 1_000_000 else raw
                break
            except ValueError:
                continue
        if market_cap is None:
            for top_key in ("marketValue", "totalMarketValue", "marketSum"):
                v = data.get(top_key)
                if v is None:
                    continue
                parsed = _parse_krw_compound(str(v))
                ...
                try:
                    ...
                    continue
        result["시가총액_naver"] = market_cap if market_cap is not None else np.nan
        if should_debug:
            print(f"[DEBUG-NAVER-MOBILE] {code} status={resp.status_code} "
                  f"totalInfos 필드={list(field_map.keys())} data 최상위 키="
                  f"{list(data.keys())[:30]} 결과={result}")
            if pd.isna(result["PER"]) and pd.isna(result["PBR"]):
                print(f"[DEBUG-NAVER-MOBILE] {code}: PER/PBR 모두 못 찾음 - 원본 JSON(앞 "
                      f"2000자)={resp.text[:2000]!r}")
    except Exception as e:
        if should_debug:
            print(f"[DEBUG-NAVER-MOBILE] {code} 요청/파싱 실패: {type(e).__name__}: {e}")
    return result


def _fetch_naver_valuation(code: str) -> dict:
    """yfinance가 한국 종목에는 PER/PBR/EPS/BPS를 거의 채워주지 못해(Yahoo의 KRX 커버리지 한계),
    네이버금융에서 이 값들을 보강 수집합니다.

    2026.09.14부터: 1순위로 모바일 API(_fetch_naver_valuation_mobile_api, 위 설명 참고)를
    시도하고, PER/PBR을 하나도 못 얻으면 기존 데스크톱 페이지(finance.naver.com/item/
    main.naver) 스크래핑으로 폴백합니다 - 데스크톱 페이지는 이제 Next.js로 바뀌어 PER/PBR/EPS/
    BPS 자체는 더 이상 여기서 못 얻지만, 배당수익률·시가총액·상장주식수를 라벨 텍스트 정규식으로
    뽑는 보조 로직은 여전히 시도할 가치가 있어 남겨둡니다(정규식이 페이지 전체 텍스트를 훑는
    방식이라 SPA라도 초기 서버 HTML에 해당 텍스트가 남아있으면 잡힐 수 있음). 두 시도 모두
    실패해도 예외 없이 NaN을 반환해 파이프라인은 계속 진행됩니다."""
    global _naver_debug_count
    should_debug = _naver_debug_count < _NAVER_DEBUG_LIMIT
    if should_debug:
        _naver_debug_count += 1

    result = _fetch_naver_valuation_mobile_api(code, should_debug)
    # 2026.09.14: 예전에는 PER/PBR만 확보되면 데스크톱 폴백을 건너뛰었는데, 시가총액도 함께
    # 여기서만 보강되는 값이라(모바일 API가 시가총액까지 다 채워주지 못하는 경우가 있을 수
    # 있음) PER/PBR "그리고" 시가총액까지 모두 확보된 경우에만 폴백을 건너뜁니다. 한국 종목
    # 시가총액이 전종목 확보 실패했던 사고(위 함수 설명 참고) 재발을 막기 위한 조치입니다.
    if pd.notna(result["PER"]) and pd.notna(result["시가총액_naver"]):
        return result

    try:
        resp = requests.get(
            f"https://finance.naver.com/item/main.naver?code={code}",
            headers=dict(_BROWSER_HEADERS, Referer="https://finance.naver.com/"),
            timeout=10,
        )
        ...
        # _per/_pbr/_eps/_bps 태그는 데스크톱 페이지가 Next.js로 바뀌면서 더 이상 존재하지
        # 않을 가능성이 높지만(위 설명 참고), 혹시 일부 종목/시점에 구버전 페이지가 아직
        # 남아있을 가능성에 대비해 모바일 API가 못 채운 값만 이걸로 보강 시도합니다.
        if pd.isna(result["PER"]):
            result["PER"] = _num("_per")
        if pd.isna(result["PBR"]):
            result["PBR"] = _num("_pbr")
        if pd.isna(result["EPS"]):
            result["EPS"] = _num("_eps")
        if pd.isna(result["BPS"]):
        ...
        if should_debug:
            print(f"[DEBUG-NAVER] {code} status={resp.status_code} 응답길이={len(resp.text)} "
                  f"결과={result}")
            if pd.isna(result["PER"]):
                print(f"[DEBUG-NAVER] {code}: _per 태그를 못 찾음(모바일 API도 실패한 뒤 여기로 "
                      f"폴백된 상태). 페이지 앞부분={resp.text[:800]!r}")
            if pd.isna(result["DIV"]) and pd.isna(result["시가총액_naver"]):
                print(f"[DEBUG-NAVER] {code}: 배당수익률/시가총액 보조 필드 정규식 추출 실패 - "
                      f"페이지 라벨 문구가 예상과 다를 수 있음(참고용/보조 필드라 동작에는 영향 없음).")
    except Exception as e:
        if should_debug:
            print(f"[DEBUG-NAVER] {code} 요청 실패: {type(e).__name__}: {e}")
    return result
...
def _fetch_naver_valuations_parallel(codes, max_workers=NAVER_MAX_WORKERS) -> dict:
    ...
    return _run_parallel_with_budget(codes, _fetch_naver_valuation, dict(_EMPTY_NAVER_VALUATION),
                                      max_workers=max_workers, label="NAVER-VAL")


_profile_debug_count = 0
_PROFILE_DEBUG_LIMIT = 3


_EMPTY_NAVER_PROFILE = {"text": "", "industry": None}


def _fetch_naver_company_profile(code: str) -> dict:
    """네이버금융과 연동된 WiseReport 기업개요 페이지에서 "회사의 주요 사업/제품" 설명 문단과
    WICS(WISE Industry Classification Standard) 업종 분류를 함께 스크래핑합니다. yfinance의
    longBusinessSummary/industry는 한국 종목에는 거의 채워지지 않아(Yahoo의 KRX 커버리지
    한계), 이 경로로 별도 보강합니다.
    사이트 구조가 바뀌면 실패할 수 있으므로(다른 네이버 스크래핑 함수들과 동일한 위험), 실패 시
    빈 값을 반환할 뿐 파이프라인을 중단시키지 않습니다.

    2026.09.14 (3): "<title>온라인기업분석 - 기업개요(SK하이닉스)</title>"의 "기업개요"와
    혼동하는 첫 번째 버그를 <title>/<script>/<style> 제외 + 정확히 "기업개요"인 소제목만
    찾도록 고쳤으나, 실제 라이브 페이지(Claude in Chrome으로 navercomp.wisereport.co.kr을
    직접 열어 원본 응답을 확인함 - 사용자 컴퓨터의 브라우저를 통해 이 세션의 프록시 차단을
    우회해 실제 페이지 구조를 검증한 첫 사례)로 재현해보니 **두 번째 버그**가 있었습니다:
    소제목을 찾은 뒤 실제 내용을 찾으려고 `node.find_next([...])`을 쓰면서 매칭 안 되면
    `node = node.parent` 후 다시 시도하는 방식이었는데, BeautifulSoup의 `find_next()`는
    "이 태그 이후의 다음 요소"가 아니라 "이 태그의 자기 자신을 포함한 하위 트리 내부부터
    시작해서 문서 순서상 다음 요소"를 찾습니다 - 즉 부모로 올라가도 여전히 그 부모의 첫
    번째 자손(원래 찾았던 그 근처의 짧은 날짜 텍스트)만 반복해서 찾게 되어, 6번을 올라가도
    실제 사업개요 문단(더 아래 있는 형제 섹션)에는 절대 도달하지 못했습니다(직접 재현해
    한 줄씩 확인함). **올바른 수정**: `find_next()`가 아니라 `find_next_sibling()`을 써서
    "같은 레벨의 다음 형제 요소"만 보도록 바꿨습니다 - 그러면 부모로 올라갈 때마다 진짜로
    그 조상 블록 전체를 건너뛰고 다음 섹션(실제 사업개요가 담긴 `<div class="cmp_comment">`)
    으로 넘어갑니다. 한화에어로스페이스(012450)·삼성전자(005930) 두 종목의 실제 응답으로
    직접 검증(둘 다 hop=4에서 정상 추출 확인).

    2026.09.14 (3) 추가: 같은 페이지 상단에 `<dt class="line-left">WICS : 우주항공과국방</dt>`
    형태로 WICS 업종 분류가 이미 나와 있는 것을 확인해, 추가 요청 없이 함께 추출합니다(예:
    한화에어로스페이스 = "우주항공과국방"). 지금까지 한국 종목의 "업종" 필드는 항상 비어
    있었는데(yfinance 없이는 대체 소스가 없다고 알고 있었음), 이 값을 채우면 국가정책 연계
    테마 매칭(_match_policy_themes)이 종목명+사업개요 텍스트뿐 아니라 공식에 가까운 업종
    분류로도 가능해져 정확도가 올라갑니다(예: "방위산업" 테마가 종목명에 "방산"이 들어가는
    경우가 거의 없어 사업개요 텍스트에만 의존했었는데, WICS "우주항공과국방" 분류가 있으면
    사업개요 추출이 실패하는 경우에도 매칭 가능)."""
    global _profile_debug_count
    should_debug = _profile_debug_count < _PROFILE_DEBUG_LIMIT
    if should_debug:
        _profile_debug_count += 1
    text = ""
    industry = None
    try:
        resp = requests.get(
        ...
        # WICS 업종(예: "WICS : 우주항공과국방") - 사업개요와 별개로 페이지 상단에 항상 있음
        wics_dt = soup.find(string=re.compile(r"^\s*WICS\s*[:：]"))
        if wics_dt:
            m = re.search(r"WICS\s*[:：]\s*(.+)", wics_dt)
            if m:
        ...
        for tag in soup.find_all(["script", "style"]):
            tag.extract()
        header = None
        for tag in soup.find_all(["th", "td", "span", "div", "b", "strong", "p", "h3", "h4"]):
            t = tag.get_text(strip=True)
            if re.fullmatch(r"기업개요\s*[:：]?", t):
                header = tag
                break
        if header is not None:
            ...
            while node is not None and hops < 8:
                # find_next_sibling: 같은 레벨의 다음 형제 요소만 봄(find_next는 자기
                # 하위 트리부터 다시 훑어 조상으로 올라가도 제자리걸음하는 버그가 있었음 -
                # 위 docstring 참고)
                sib = node.find_next_sibling(["td", "div", "p", "li", "ul"])
                if sib is not None:
        ...
        if should_debug:
            print(f"[DEBUG-PROFILE] {code} status={resp.status_code} 응답길이={len(resp.text)} "
                  f"추출길이={len(text)} WICS업종={industry!r}")
            if not text:
                print(f"[DEBUG-PROFILE] {code}: 사업개요 추출 실패. 페이지 앞부분="
                      f"{resp.text[:500]!r}")
    except Exception as e:
        if should_debug:
            print(f"[DEBUG-PROFILE] {code} 요청 실패: {type(e).__name__}: {e}")
    return {"text": text, "industry": industry}


def _fetch_naver_profiles_parallel(codes, max_workers=NAVER_MAX_WORKERS) -> dict:
    """여러 종목의 국문 사업개요 + WICS 업종을 동시에(병렬) 수집합니다. 원리는
    _fetch_naver_valuations_parallel과 동일합니다(순차 요청 시 타임아웃 위험 + 시간 예산 적용)."""
    return _run_parallel_with_budget(codes, _fetch_naver_company_profile, dict(_EMPTY_NAVER_PROFILE),
...
def _parse_naver_financial_table(resp_text: str) -> dict:
    ...
    for i, label in first_col.items():
        ...
        if pd.isna(result["ROE"]) and "ROE" in label.upper():
            # 표의 ROE는 "12.5"처럼 이미 퍼센트 그 자체인데, 화면 표시 함수 _fmt_pct()는 이
            # 프로젝트의 다른 모든 성장률/비율 컬럼과 마찬가지로 "비율"(0.125=12.5%)을 받아
            # 내부에서 다시 ×100을 합니다. 그대로 저장하면 화면에 12.5%가 아니라 1250.0%로
            # 100배 부풀려 표시되는 단위 중복 버그가 나 ÷100으로 비율로 정규화합니다(2026.09.13
            # 라이브 점검에서 삼성전자 ROE가 "+3138.0%"로 뜨는 것으로 실제 확인된 버그).
            raw_roe = _last_numeric(i)
            result["ROE"] = raw_roe / 100.0 if pd.notna(raw_roe) else np.nan
            ...
            # 표의 매출액/영업이익은 "억원" 단위(예: 34,846 = 3조 4,846억원)로 표시되는데,
            # 이 프로젝트의 금액 포맷 함수 _fmt_money()·기존 시가총액_naver 필드는 모두 "원"
            # (won) 단위를 받아 화면 표시 시 ÷1억을 합니다. 억원 값을 그대로 저장하면 화면에
            # "0억원"으로 표시되는 단위 중복(반대 방향) 버그가 나(2026.09.13 라이브 점검에서
            # 매출성장률/영업이익성장률은 정상인데 매출액/영업이익만 "0억원"으로 뜨는 것으로
            # 실제 확인됨) ×1억을 해 원 단위로 맞춥니다.
            result["매출액"] = latest * 100_000_000 if pd.notna(latest) else np.nan
            result["매출성장률"] = growth
        elif pd.isna(result["영업이익"]) and label_norm.startswith("영업이익") \
                and "률" not in label_norm:
            growth, latest = _yoy_growth(i)
            result["영업이익"] = latest * 100_000_000 if pd.notna(latest) else np.nan
            result["영업이익성장률"] = growth
    return result
...
def _fetch_naver_financial_ratio(code: str) -> dict:
    ...
    headers_base = _BROWSER_HEADERS  # _fetch_naver_valuation에 정의된 공용 브라우저 UA 재사용
...
def fetch_kr_fundamentals_yfinance(markets=None) -> pd.DataFrame:
    ...
    print(f"[KR] 사업개요 수집 완료 ({sum(1 for v in profile_map.values() if v.get('text'))}개 "
          f"종목에서 텍스트 확보, WICS 업종 확보 "
          f"{sum(1 for v in profile_map.values() if v.get('industry'))}개)")
    ...
    for _, r in universe.iterrows():
        ...
        profile_entry = profile_map.get(code) or _EMPTY_NAVER_PROFILE
        profile_text = profile_entry.get("text") or info.get("longBusinessSummary") or ""
        # 업종: yfinance 우선(영문 GICS, 이미 검증된 소스), 없으면 네이버 WICS 업종으로 보강
        industry = info.get("industry") or profile_entry.get("industry")
        rows.append({
...
def fetch_kr_fundamentals_naver(markets=None) -> pd.DataFrame:
    ...
    for _, r in universe.iterrows():
        ...
        market_cap = _to_numeric_clean(pd.Series([r.get("시가총액_참고")])).iloc[0]
        if pd.isna(market_cap):
            market_cap = naver_val.get("시가총액_naver", np.nan)

        shares = naver_val.get("상장주식수_naver", np.nan)
        profile_entry = profile_map.get(code) or _EMPTY_NAVER_PROFILE
        profile_text = profile_entry.get("text") or ""
        # 2026.09.14 (3): 이전에는 yfinance 없이는 업종을 얻을 대체 소스가 없다고 보고 항상
        # None으로 비워뒀으나, _fetch_naver_company_profile()이 같은 요청에서 WICS(WISE
        # Industry Classification) 업종 분류(예: "우주항공과국방")도 함께 추출하도록 바뀌어
        # 이제 채워집니다. _match_policy_themes가 종목명+사업개요 텍스트 외에 이 값으로도
        # 매칭할 수 있어 정확도가 올라갑니다.
...
def fetch_kr_momentum(tickers, lookback_days=380):
    ...
    for t in tickers:
        try:
            ...
            continue
        time.sleep(0.05)
    # 2026.09.14: tickers를 하나도 처리 못하면(전종목 fdr.DataReader 실패 등 - 예를 들어
    # 시가총액 파이프라인 자체가 깨진 비상 상황이면 같은 FinanceDataReader가 여기서도 전부
    # 실패할 가능성이 높음) rows=[]가 되어 pd.DataFrame([])는 컬럼이 하나도 없는 완전히 빈
    # DataFrame이 됩니다. 이걸 build_kr_universe()에서 df.merge(momentum, on="티커", ...)로
    # 병합하면 오른쪽에 "티커" 컬럼 자체가 없어 KeyError('티커')로 크래시합니다(이번에 새로
    # 추가한 비상 우회 경로가 오히려 이 크래시를 다시 유발할 뻔했음 - 테스트로 재현 후 발견).
    # 데이터가 없을 때도 병합에 필요한 컬럼 구조만은 항상 보장해 이 크래시를 막습니다.
    if not rows:
        return pd.DataFrame(columns=[
            "티커", "수익률_3M", "수익률_6M", "수익률_12M",
            "거래량", "거래대금", "거래기준일", "현재주가_모멘텀",
        ]), price_history
...
def build_kr_universe():
    ...
    cap_valid = pd.to_numeric(fundamentals.get("시가총액"), errors="coerce")
    cap_ok_count = int(cap_valid.notna().sum())
    total_count = len(fundamentals)
    print(f"[KR] 시가총액 최종 필터 직전: {total_count}개 종목 중 시가총액 확보 "
          f"{cap_ok_count}개, {KR_MIN_MARKETCAP:,.0f}원 이상 "
          f"{int((cap_valid >= KR_MIN_MARKETCAP).sum())}개")

    # 2026.09.14: FinanceDataReader/네이버 양쪽 모두 시가총액을 거의 못 가져오는 장애(원인
    # 조사 중, get_kr_ticker_universe의 진단 로그 참고)가 실제로 있었는데, 이때 "시가총액
    # 1,000억 이상"이라는 원래 스코프를 원칙대로 지키면 한국 종목이 전부(0개) 걸러져 대시보드
    # 에서 한국 주식이 통째로 사라지는 결과가 됩니다. 사용자 입장에서는 "일부 부정확할 수 있는
    # 목록"보다 "아예 아무것도 안 보이는 것"이 훨씬 나쁜 경험이라, 시가총액을 거의 다 못 얻은
    # (확보율 5% 미만) 비상 상황에서는 이 필터를 건너뜁니다. 다만 전종목(코스피+코스닥 약
    # 2,700여 개)을 그대로 다 통과시키면 뒤이은 fetch_kr_momentum()이 종목당 순차 요청이라
    # (이 프로젝트가 이미 여러 번 겪은 GitHub Actions 45분 타임아웃의 원인) 실행 시간이 크게
    # 늘어날 위험이 있어, 시가총액 대신 참고할 수 있는 다른 규모 지표가 없는 상황에서도 최소한
    # 개수만은 KR_EMERGENCY_MAX_UNIVERSE로 제한합니다(순서는 리스트 원본 순서 그대로라 대형주
    # 위주로 잡힌다는 보장은 없음 - "정확한 대형주 목록"이 아니라 "완전히 텅 빈 것보다는 나은
    # 임시 목록"이라는 점을 감안할 것).
    cap_pipeline_broken = total_count > 0 and (cap_ok_count / total_count) < 0.05
    if cap_pipeline_broken:
        df = fundamentals.copy()
        if len(df) > KR_EMERGENCY_MAX_UNIVERSE:
            df = df.head(KR_EMERGENCY_MAX_UNIVERSE)
        print(f"[KR] 시가총액 확보율이 {cap_ok_count}/{total_count}로 너무 낮아(데이터 소스 "
              f"장애로 추정) 시가총액 필터를 이번 실행에서는 건너뛰고 {len(df)}개 종목으로 "
              f"진행합니다(대형주 위주라는 보장 없음, 실행 시간 보호를 위해 최대 "
              f"{KR_EMERGENCY_MAX_UNIVERSE}개로 제한) - 근본 원인은 get_kr_ticker_universe()의 "
              f"'cap_col=...' 진단 로그를 확인하세요.")
    else:
        df = fundamentals[cap_valid >= KR_MIN_MARKETCAP].copy()
    for kw in KR_EXCLUDE_KEYWORDS:
        df = df[~df["종목명"].astype(str).str.contains(kw, na=False)]

    # 2026.09.14: df가 텅 비면(예: 시가총액을 단 한 종목도 못 얻은 경우) 빈 DataFrame에
    # .apply(axis=1)을 호출했을 때 pandas가 결과 타입을 추론하지 못해 빈 float64 Series를
    # 반환하고, 이걸로 df[~is_preferred]를 하면 "불리언 마스크로 행을 거르는 것"이 아니라
    # "컬럼 이름 목록으로 컬럼을 선택하는 것"으로 잘못 해석돼 KeyError('PER')가 나는 pandas의
    # 알려진 함정이 있습니다(실제 라이브 실행에서 이 크래시로 하루 전체 실행이 실패한 것을
    # 사용자가 공유한 GitHub Actions 로그로 확인). 빈 경우를 먼저 걸러 이 함정을 피합니다.
    if df.empty:
        print("[KR] 시가총액 필터를 통과한 종목이 하나도 없습니다 - 이번 실행에서는 한국 종목"
              "없이(빈 결과로) 진행합니다. 위 '시가총액 확보 N개' 줄이 0에 가까우면 "
              "FinanceDataReader/KIND 종목 리스트에서 시가총액 자체를 못 얻었다는 뜻입니다.")
        empty_cols = list(fundamentals.columns) + ["수익률_3M", "수익률_6M", "수익률_12M",
                                                     "거래량", "거래대금", "거래기준일", "시장구분"]
...
def run(kr: bool = True, us: bool = True) -> str:
    ...
    if kr:
        print("[KR] 데이터 수집 중...")
        # 2026.09.14: 한국 데이터 수집(네이버/FinanceDataReader 등 외부 사이트 의존)에서 예기치
        # 못한 예외가 나도 미국 쪽 결과까지 통째로 날아가지 않도록 감쌉니다 - 실제로 이 try/except가
        # 없었을 때, 한국 쪽에서만 난 예외(KeyError: 'PER', 아래 build_kr_universe의 빈
        # DataFrame 방어 로직 참고) 때문에 GitHub Actions 실행 전체가 실패해 미국 쪽까지 포함해
        # 대시보드가 아예 갱신되지 않는 하루 전체 장애로 이어진 것을 사용자가 공유한 로그로 확인.
        try:
            ...
            print(f"[KR] 한국 데이터 수집/처리 중 예외 발생 - 한국 종목 없이 계속 진행합니다 "
                  f"(미국 쪽은 정상 진행): {type(e).__name__}: {e}")
            traceback.print_exc()

...
def fetch_kr_momentum(tickers, lookback_days=380):
    ...
    최근 20거래일 평균 거래량/거래대금, 그리고 관심종목 추적(과거 날짜 지정 시 그날의 실제
    종가를 기준가로 쓰기 위한) 일별 종가 이력을 함께 수집합니다. 추가 네트워크 호출 없이
    기존에 이미 하던 시세 조회 결과를 재활용하는 것이라 실행 시간에 미치는 영향은 없습니다."""
    if fdr is None:
        raise ImportError("FinanceDataReader가 설치되어 있지 않습니다.")
    end = dt.date.today()
    ...
    for t in tickers:
        try:
            ...
            recent_px = px.reindex(recent_vol.index)
            avg_volume = float(recent_vol.mean()) if len(recent_vol) else np.nan
            avg_value = float((recent_px * recent_vol).mean()) if len(recent_vol) and recent_px.notna().any() else np.nan

            rows.append({
                "티커": t, "수익률_3M": ret(90), "수익률_6M": ret(180), "수익률_12M": ret(365),
                "거래량_20일평균": avg_volume, "거래대금_20일평균": avg_value,
            })
...
def fetch_us_momentum(tickers):
    """모멘텀과 함께, 이미 받아온 일별 시세에서 최근 20거래일 평균 거래량/거래대금과
    관심종목 추적용 일별 종가 이력을 추가 네트워크 호출 없이 함께 수집합니다."""
    if yf is None:
        raise ImportError("yfinance가 설치되어 있지 않습니다.")
    rows = []
    price_history = {}
    for t in tickers:
        try:
            ...
            vol = hist_df["Volume"].dropna() if "Volume" in hist_df.columns else pd.Series(dtype=float)
            recent_vol = vol.tail(20)
            recent_px = hist.reindex(recent_vol.index)
            avg_volume = float(recent_vol.mean()) if len(recent_vol) else np.nan
            avg_value = float((recent_px * recent_vol).mean()) if len(recent_vol) and recent_px.notna().any() else np.nan
...
def _card(row, show_fair_value=False, show_volume=False) -> str:
    ...
    if show_fair_value:
        ...
        value_str = _fmt_money(row.get("거래대금_20일평균"), market)
        vol = row.get("거래량_20일평균")
        vol_str = f"{vol:,.0f}주" if pd.notna(vol) else "-"
        extra_metrics = f'<span>거래대금(20일) {value_str}</span><span>거래량(20일) {vol_str}</span><span>12개월 {m12}</span>'
...
def build_dashboard(df: pd.DataFrame, price_history: dict = None, out_path: str = "docs/index.html") -> str:
    ...
    volume = _section("volume", "\U0001F4B9 거래대금 TOP 20 (전체 종목 대상, 추천점수와 무관)",
                       df, "거래대금_20일평균", 20, show_volume=True)
...
"거래대금 TOP"은 추천점수와 무관하게, 최근 20거래일 평균 거래대금(가격×거래량) 기준으로 코스피·
코스닥·S&amp;P500 전체에서 현재 가장 활발히 거래되는 종목을 모은 목록입니다 - 저희 추천 리스트에
없는 종목도 여기서 확인할 수 있습니다.<br><br>

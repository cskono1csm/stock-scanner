...
상단 검색창에서는 저희가 매일 수집하는 전체 종목(코스피·코스닥 시가총액 1,000억원 이상 +
S&amp;P500) 중 종목명·티커로 찾아 바로 세부정보를 볼 수 있습니다 - 추천 리스트(TOP 15/20)에
없는 종목도 검색되면 조회 가능합니다. 다만 이 수집 대상 밖의 코스피·코스닥 소형주나
나스닥의 S&amp;P500 미편입 종목은 검색되지 않습니다.</footer>
...
// 종목 검색: 서버가 매일 수집해두는 전체 종목 가격표(PRICES, 코스피/코스닥 + S&P500)를
// 그대로 검색 대상으로 씁니다 - 카드로 추천되지 않은 종목도 이 안에 있으면 검색·조회할 수
// 있습니다(다만 저희가 매일 수집하는 유니버스, 즉 코스피/코스닥 시가총액 1,000억원 이상 +
// S&P500 밖의 종목은 검색되지 않습니다).

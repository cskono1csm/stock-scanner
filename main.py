...
def _fetch_naver_valuation_mobile_api(code: str, should_debug: bool) -> dict:
    ...
    try:
        ...
        # 시가총액도 이 응답 어딘가에 있을 가능성이 높아 최선을 다해 추정 시도합니다(정확한
        # 필드명 미확인 - totalInfos 코드값 후보 + data 최상위 후보 키 둘 다 시도). 2026.09.14:
        # FinanceDataReader 쪽 시가총액도 전종목 확보 실패 중이라 이 값이 특히 중요해졌습니다.
        market_cap = _f("marketvalue", "totalmarketvalue", "marcap", "aftermarketvalue")
        if pd.isna(market_cap):
            for top_key in ("marketValue", "totalMarketValue", "marketSum"):
                v = data.get(top_key)
                if v is not None:
                    try:
                        market_cap = float(str(v).replace(",", ""))
                        break
                    except (TypeError, ValueError):
                        continue
        # totalInfos 값은 보통 "억원" 단위 문자열(예: "5,171,234")로 오는 경우가 많아, 자릿수가
        # 시가총액치고 비정상적으로 작으면(1e6 미만) 억원 단위로 보고 원 단위로 환산합니다 -
        # 반대로 이미 원 단위로 온 경우엔 건드리지 않습니다. 다음 로그로 실제 스케일을 확인해
        # 이 추정이 맞는지 검증할 것.
        if pd.notna(market_cap) and market_cap < 1_000_000:
            market_cap = market_cap * 100_000_000
        result["시가총액_naver"] = market_cap
        if should_debug:

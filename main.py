...
# ROE/부채비율(상세팝업 참고용, 스코어링에는 안 씀)을 별도 엔드포인트(companyinfo.stock.
# naver.com)에서 추가로 시도할지 여부. 이 엔드포인트는 이 프로젝트에서 이번에 처음 시도하는
# 것이라 라이브 검증이 안 됐고, 2026.09.10 KR_DATA_SOURCE="naver" 첫 배포 직후 실행이 다시
# 45분 타임아웃으로 실패해(원인 조사 중) 일단 기본값을 꺼둡니다. PER/PBR/EPS/BPS/배당수익률/
# 사업개요(모두 라이브 검증된 경로)만으로도 핵심 추천 점수는 정상 동작하므로, 원인이 명확히
# 확인되기 전까지는 이 추가 조회 자체를 하지 않는 쪽이 안전합니다. 원인이 이 엔드포인트가
# 아니었던 것으로 확인되면 True로 켤 수 있습니다(시간 예산 안전장치는 이미 적용돼 있음).
KR_FETCH_FINANCIAL_RATIOS = False
...
def _fetch_naver_financial_ratio(code: str) -> dict:
    """네이버금융 산하 기업정보(companyinfo.stock.naver.com)의 재무비율/실적 표에서
    ROE·부채비율을 참고용으로 수집합니다.

    2026.09.10부터 한국 종목 펀더멘털을 yfinance 대신 네이버금융으로만 수집하게 되면서
    (KR_DATA_SOURCE="naver" 기본값, fetch_kr_fundamentals_naver 참고), ROE/부채비율은
    PER/PBR과 같은 페이지(finance.naver.com/item/main.naver)에는 없어 별도 엔드포인트가
    필요합니다. 커뮤니티에 공개된 크롤링 예시(예: financedata.github.io 블로그의 네이버금융
    재무제표 크롤링 글)를 참고해 아래 엔드포인트를 씁니다. 이 세션은 실제 응답을 라이브로
    받아볼 수 없어(외부망 차단) 표 구조를 확인하지 못했습니다 - ROE/부채비율은 스코어링/추천
    순위에는 전혀 쓰이지 않는 "참고 표시용" 필드라, 이 함수가 완전히 실패해도(표 구조가 예상과
    다르거나 엔드포인트가 바뀐 경우) NaN만 반환할 뿐 파이프라인 동작에는 영향이 없습니다."""
    global _ratio_debug_done
    result = {"ROE": np.nan, "부채비율": np.nan}
    try:
        resp = requests.get(
        ...
        if not _ratio_debug_done:
            ...
            print(f"[DEBUG-RATIO] {code} status={resp.status_code} 결과={result} "
                  f"(참고용 필드 - 실패해도 추천 순위에는 영향 없음)")
            if pd.isna(result["ROE"]) and pd.isna(result["부채비율"]):
                print(f"[DEBUG-RATIO] ROE/부채비율 행을 못 찾음. 응답 앞부분={resp.text[:500]!r}")
    except Exception as e:
        if not _ratio_debug_done:
            _ratio_debug_done = True
            print(f"[DEBUG-RATIO] {code} 요청/파싱 실패(참고용 필드라 무시하고 계속 진행): {e}")
    return result


def _fetch_naver_financial_ratios_parallel(codes, max_workers=NAVER_MAX_WORKERS) -> dict:
    """여러 종목의 ROE/부채비율(참고용)을 동시에(병렬) 수집합니다. 원리는
    _fetch_naver_valuations_parallel과 동일합니다(시간 예산 적용).
    KR_FETCH_FINANCIAL_RATIOS=False면 fetch_kr_fundamentals_naver()에서 아예 호출하지
    않습니다(이 함수가 쓰는 엔드포인트는 라이브 검증이 안 돼 있어 기본값은 꺼둠)."""
    empty = {"ROE": np.nan, "부채비율": np.nan}
    return _run_parallel_with_budget(codes, _fetch_naver_financial_ratio, empty,
...
def fetch_kr_fundamentals_naver(markets=None) -> pd.DataFrame:
    ...
    if KR_FETCH_FINANCIAL_RATIOS:
        print(f"[KR] 재무비율(ROE/부채비율, 참고용) 병렬 수집 시작 ({len(universe)}개 종목)")
        ratio_map = _fetch_naver_financial_ratios_parallel(universe["티커"].tolist())
        roe_found = sum(1 for v in ratio_map.values() if pd.notna(v.get("ROE")))
        print(f"[KR] 재무비율 수집 완료(ROE 확보 {roe_found}개 - 참고용, 스코어링에는 미반영)")
    else:
        # 라이브 검증이 안 된 엔드포인트라 기본적으로 꺼둠(CONFIG의 KR_FETCH_FINANCIAL_RATIOS
        # 참고) - ROE/부채비율은 상세팝업 참고용일 뿐 추천 순위 계산에는 쓰이지 않아 꺼도 무방
        print("[KR] 재무비율(ROE/부채비율) 수집은 이번 실행에서 비활성화 상태입니다 "
              "(CONFIG의 KR_FETCH_FINANCIAL_RATIOS=True로 바꾸면 다시 시도합니다).")
        ratio_map = {}
    ...
    empty_ratio = {"ROE": np.nan, "부채비율": np.nan}
    rows = []
    for _, r in universe.iterrows():
        code, name, market = str(r["티커"]), r["종목명"], r["시장"]
        naver_val = naver_map.get(code, empty_naver)
...
사라집니다).<br><br>
위 카테고리 탭(종합/가치주/모멘텀/배당/적정주가/거래대금/관심종목)을 누르면 해당 목록만 바로 볼 수 있습니다.<br><br>

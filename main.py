...
def _fetch_naver_profiles_parallel(codes, max_workers=10) -> dict:
    """여러 종목의 국문 사업개요를 동시에(병렬) 수집합니다. 원리는
    _fetch_naver_valuations_parallel과 동일합니다(순차 요청 시 타임아웃 위험)."""
    results = {}
    codes = list(dict.fromkeys(str(c) for c in codes))
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        future_map = {executor.submit(_fetch_naver_company_profile, code): code for code in codes}
        for future in concurrent.futures.as_completed(future_map):
            code = future_map[future]
            try:
                results[code] = future.result()
...
def _fetch_naver_financial_ratios_parallel(codes, max_workers=10) -> dict:
    """여러 종목의 ROE/부채비율(참고용)을 동시에(병렬) 수집합니다. 원리는
    _fetch_naver_valuations_parallel과 동일합니다."""
    results = {}
    codes = list(dict.fromkeys(str(c) for c in codes))
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        future_map = {executor.submit(_fetch_naver_financial_ratio, code): code for code in codes}
        for future in concurrent.futures.as_completed(future_map):
            code = future_map[future]
            try:
                results[code] = future.result()
            except Exception:
                results[code] = {"ROE": np.nan, "부채비율": np.nan}
    return results

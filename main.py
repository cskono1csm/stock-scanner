...
# 관심종목 추적 UI(카드 탭 -> 추적 시작)는 오직 이 5개 TOP 리스트에 실제로 뜬 카드에서만
# 열 수 있어("추천된 종목 중에서만 선택"), 여기서는 그 카드가 "어떤 근거로 추천됐는지" 라벨만
...
def _build_prices_json(df: pd.DataFrame) -> str:
...
def build_dashboard(df: pd.DataFrame, out_path: str = "docs/index.html") -> str:
    ...
    prices_json = _build_prices_json(df)
...
<main>{total}{value}{mom}{div_}{fair}<section id="performance"><h2>\U0001F4CC 관심종목 추적</h2><div id="perfContainer"></div></section></main>
...
추천된 종목 카드를 탭하면 회계·재무 세부정보와 함께 "관심종목 추적"을 시작할 수 있습니다 - 이 종목이
어느 리스트(종합/가치주/모멘텀/배당/적정주가)에서 추천됐는지와 그날의 가격을 함께 기록해 두어,
나중에 실제로 분석한 방향대로 주가가 움직였는지 확인하는 용도입니다. 추적 목록은 이 기기의
브라우저에만 저장됩니다(서버에는 저장되지 않아 다른 기기/브라우저에는 보이지 않고, 브라우저
데이터를 지우면 사라집니다).<br><br>
위 카테고리 탭(종합/가치주/모멘텀/배당/적정주가/관심종목)을 누르면 해당 목록만 바로 볼 수 있습니다.</footer>
...
function addToWatchlist(key, date) {{
  ...
  w[key] = {{
    ...
    추천구분: info.추천구분 || '', 추적일: date || todayStr(), 추적가: info.현재주가
  }};
  saveWatchlist(w);
...
function renderTrackControls(key) {{
  ...
  if (entry) {{
    ...
    el.innerHTML =
      '<div class="track-box"><label class="track-label">추적 시작일(임의로 변경 가능, 기준가격은 오늘자 현재가)</label>' +
      '<div class="track-add-row"><input type="date" id="trackDateInput" value="' + today + '" max="' + today + '">' +

...
def _fetch_yfinance_info_parallel(symbols, max_workers=8) -> dict:
    """여러 종목의 yfinance 펀더멘털(get_info)을 동시에(병렬) 수집합니다. 기존에는 종목별
    순차 요청(+0.05초 대기)이라 1,300여 개 기준 실행 시간이 컸고, 특히 2026.09에 Yahoo
    Finance 쪽에서 대량의 "Quote not found" 404가 발생했을 때 순차 요청이라 실패까지도
    하나씩 기다리며 처리하느라 45분 타임아웃을 다시 넘기는 사고로 이어졌습니다. 네이버금융
    병렬 수집과 같은 방식으로 스레드풀을 써서, 성공이든 실패든 총 소요 시간을 크게 줄입니다."""
    results = {}
    fail_count = 0
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for future in concurrent.futures.as_completed(future_map):
            ...
            if info:
                ...
                fail_count += 1
    total = len(symbols)
    print(f"[YF] {total}개 요청 -> 성공 {len(results)}개 / 실패 {fail_count}개")

...
def _fetch_naver_valuation(code: str) -> dict:
    ...
    (정확한 <em id="..."> 태그로 파싱) 외에, 배당수익률·시가총액·상장주식수도 같은 페이지에서
    함께 추출합니다(추가 요청 없음). 다만 이 3개는 정확한 태그 id를 이 세션에서 확인할 방법이
    없어(라이브 접근 불가), 태그 구조가 바뀌어도 잘 견디도록 "라벨 텍스트 다음에 숫자가 나온다"
    는 패턴을 페이지 전체 텍스트에서 정규식으로 찾는 방식을 씁니다. 실패해도 예외 없이 NaN을
    반환해 파이프라인은 계속 진행됩니다.

    2026.09.13: 브라우저 User-Agent 헤더 추가(아래 _BROWSER_HEADERS 설명 참고) + 진단 로그를
    첫 3개 종목까지로 확대."""
    ...
    try:
        ...
        if should_debug:
            ...
            if pd.isna(result["PER"]):
                print(f"[DEBUG-NAVER] {code}: _per 태그를 못 찾음. 페이지 앞부분="
                      f"{resp.text[:800]!r}")

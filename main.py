...
def _fetch_naver_company_profile(code: str) -> str:
    ...
    빈 문자열을 반환할 뿐 파이프라인을 중단시키지 않습니다.
    2026.09.13: 브라우저 User-Agent 헤더 추가(_fetch_naver_valuation의 _BROWSER_HEADERS 설명
    참고 - 2026.09.13 라이브 점검에서 이 함수도 거의 모든 종목에서 사업개요를 못 가져오고 있는
    것이 확인됨) + 진단 로그를 첫 3개 종목까지로 확대.

    2026.09.14 (3): 실제 GitHub Actions 로그로 원인 확정 - 응답 자체는 정상(실제 페이지가
    옴, 예: 응답길이 84,967, "<title>온라인기업분석 - 기업개요(SK하이닉스)</title>" 같은
    진짜 기업개요 페이지)인데도 1,312개 종목 전부에서 추출 결과가 0글자였습니다. 원인은
    `soup.find(string=re.compile("기업개요"))`가 문서에서 "기업개요"라는 글자가 처음
    등장하는 위치를 찾는데, 그게 거의 항상 <title> 태그 안(예: "...- 기업개요(SK하이닉스)")
    이었던 것 - <title>의 부모는 <head>뿐이라 `find_parent([...])`가 매번 None을 반환해
    루프 자체가 실행되지 못하고 text가 항상 빈 문자열로 남았습니다(사이트 구조 문제가 아니라
    순전히 이 코드의 검색 로직 버그). <title>/<script>/<style> 안의 텍스트는 애초에 대상에서
    제외하고, "기업개요"라는 글자를 "포함"하는 아무 텍스트가 아니라 실제 소제목처럼 그 글자
    자체(앞뒤 공백/콜론 정도만 허용)로만 이루어진 태그를 찾도록 바꿨습니다."""
